...
Per eseguire un comando supplementare sul plug-in SnapCenter per MySQL, è necessario includerlo nel `allowed_commands.config` file.

`allowed_commands.config` Il file si trova nella sottodirectory "etc" della directory SnapCenter Plug-in for MySQL.
