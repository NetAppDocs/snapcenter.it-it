...
NOTE: Il plug-in per MySQL deve essere installato sul database server MySQL.
...
+
Mentre si utilizza l'API REST per installare Plug-in per MySQL, è necessario passare la versione come 3,0. Ad esempio, MySQL:3,0
...
Percorso di installazione
 a| 
Il plug-in per MySQL è installato sull'host client MySQL, e questo host può essere su un sistema Windows o Linux.
...
.Al termine
Se si desidera eseguire l'aggiornamento alla versione SnapCenter 6,0, il plug-in basato su PERL per MySQL verrà disinstallato dal server plug-in remoto.
