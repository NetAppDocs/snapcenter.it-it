...
* Il ripristino automatico point-in-time e up-to-minute non è supportato.
...
. Nella pagina Restore Scope, selezionare *complete Resource* per ripristinare i volumi di dati configurati del database MySQL.
...
NOTE: Per i comandi pre e post per le operazioni quiesce, snapshot e unquiesce, è necessario controllare se i comandi sono presenti nell'elenco dei comandi disponibile sull'host plug-in dal percorso _/opt/snapcenter/snapenter/scc/allowed_commands.config_ per Linux.
