...
summary: 'Il ripristino di un backup MySQL include l"avvio di una sessione di connessione con il server SnapCenter, l"elenco dei backup e il recupero delle informazioni di backup e il ripristino di un backup.' 
---
= Ripristinare il database MySQL utilizzando i cmdlet PowerShell
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Il ripristino di un backup MySQL include l'avvio di una sessione di connessione con il server SnapCenter, l'elenco dei backup e il recupero delle informazioni di backup e il ripristino di un backup.
...
NOTE: AppObjectId è "host\Plugin\UID", dove UID = <instance_name> è per la risorsa di istanza MySQL rilevata manualmente e UID = <instance_name>\<databse_name> è per la risorsa di database MySQL. È possibile ottenere ResourceID dal cmdlet Get-SmResources.
