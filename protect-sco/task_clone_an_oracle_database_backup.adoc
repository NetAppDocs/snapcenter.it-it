...
+
È possibile aggiungere altri parametri facendo clic su * *image:../media/add_policy_from_resourcegroup.gif["aggiungere il gruppo di risorse del modulo criterio"].
...
== Aggiornare l'IP preferito sull'host

Al termine dell'operazione di clonazione, il percorso fornito dal livello di accesso allo storage (SAL) al clone sarà in formato _<nfs_lif_IP>:<JunctionPath>_. Per fornire l'IP preferito, è necessario configurarlo sull'host utilizzando i comandi SCCLI.

.Fasi
. Accedere all'host del database.
. Avvio di una sessione di connessione con SnapCenter per un utente specifico.
+
Open-SmConnection

. Creare un file vuoto.
+
toccare /var/opt/SnapCenter/scu/etc/storagepreference.properties

. Configura la LIF dati preferita per la SVM.
+
Add-SvmPreferredDataPath -SVM <SVM Name> -DataPath <indirizzo IP o FQDN>
