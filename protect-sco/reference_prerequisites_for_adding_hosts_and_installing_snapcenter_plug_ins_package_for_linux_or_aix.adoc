...
* Devi aver installato Java 17 (per SnapCenter 6.2.1 e versioni successive) o Java 11 (per SnapCenter 6.2 e versioni precedenti) sul tuo host Linux o AIX.
+
** Java di Oracle, OpenJDK o SAP JDK è supportato per Linux.
** IBM Java per AIX. È possibile scaricarlo da  https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["IBM Semeru Runtimes Download"^].
+
...
* Modifica il  `/etc/ssh/sshd_config` file per configurare gli algoritmi del codice di autenticazione del messaggio: MACs hmac-sha2-256 e MACs hmac-sha2-512.
+
...
Pacchetti software richiesti
 a| 
* IBM Java 17 per SnapCenter 6.2.1 e versioni successive oppure IBM Java 11 per SnapCenter 6.2 e versioni precedenti
...
CAUTION: Sebbene sia consentito creare le credenziali dopo aver distribuito gli host e installato i plug-in, la *best practice* è quella di crearle dopo aver aggiunto gli SVM e prima di distribuire gli host e installare i plug-in.
...
NOTE: Se durante la creazione delle credenziali si inseriscono dati errati, viene visualizzato un messaggio di errore. È necessario selezionare *Annulla* e poi riprovare.
...
. Seleziona image:../media/filter_icon.gif["icona del filtro"], quindi seleziona l'host name e il tipo di database per filtrare le risorse.
+
A questo punto è possibile selezionare image:../media/filter_icon.gif["icona del filtro"] per chiudere il riquadro dei filtri.

. Seleziona il database, quindi seleziona *Impostazioni database* > *Configura database*.
