...
summary: 'Il ripristino di un backup IBM DB2 include l"avvio di una sessione di connessione con il server SnapCenter, l"elenco dei backup e il recupero delle informazioni di backup e il ripristino di un backup.' 
---
...
NOTE: AppObjectId è "host\Plugin\UID", dove UID = <instance_name> è per la risorsa di istanza DB2 rilevata manualmente e UID = <instance_name>\<databse_name> è per la risorsa di database IBM DB2. È possibile ottenere ResourceID dal cmdlet Get-smResources.
