...
summary: Prima di aggiungere un host e installare i pacchetti plug-in, è necessario completare tutti i requisiti. Il plug-in SnapCenter per IBM DB2 è disponibile sia in ambienti Windows che Linux. 
---
= Prerequisiti per aggiungere host e installare il plug-in SnapCenter per IBM DB2
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Prima di aggiungere un host e installare i pacchetti plug-in, è necessario completare tutti i requisiti. Il plug-in SnapCenter per IBM DB2 è disponibile sia in ambienti Windows che Linux.
...
* È necessario aver attivato la connessione SSH basata su password per l'utente root o non root.
* È necessario aver installato la libreria *mksh* sul proprio host Linux.
...
Per eseguire un comando supplementare sul plug-in SnapCenter per IBM DB2, è necessario includerlo nel `allowed_commands.config` file.

`allowed_commands.config` Il file si trova nella sottodirectory "etc" del plug-in SnapCenter per IBM DB2.
...
Percorso personalizzato: `<custom_Directory>/NetApp/snapcenter/scc/etc/allowed_commands.config`
