...
* Per ONTAP 9.12.1 e versioni precedenti, i cloni creati dagli snapshot SnapLock Vault come parte del ripristino erediteranno il tempo di scadenza SnapLock Vault. Gli amministratori di storage dovrebbero pulire manualmente i cloni dopo il tempo di scadenza SnapLock.
...
Ad esempio, se si aggiunge HR come tag a più gruppi di risorse, è possibile trovare in seguito tutti i gruppi di risorse associati al tag HR.
 ...
 a| 
Utilizza un formato nome personalizzato per Snapshot
 a| 
...
Selezionare *SnapCenter senza coerenza del file system*.
 ...
 a| 
Per inserire il comando da eseguire sull'host per creare snapshot.
 a| 
