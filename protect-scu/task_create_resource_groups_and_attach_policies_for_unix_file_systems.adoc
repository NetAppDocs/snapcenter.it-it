...
NOTE: Per la coerenza del file system, vengono acquisiti snapshot del gruppo di coerenza per le LUN coinvolte nel gruppo di volumi.
...
NOTE: Se sono stati aggiunti diversi file system al gruppo di risorse, tutti i volumi provenienti da file system differenti all'interno del gruppo di risorse vengono inseriti in un Consistency group. Per i backup coerenti in caso di arresto anomalo, viene creata un'istantanea del Consistency group per impostazione predefinita, anche se è presente un solo file system o volume.
