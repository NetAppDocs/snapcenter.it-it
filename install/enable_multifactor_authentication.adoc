...
* Le informazioni relative ai parametri che possono essere utilizzati con il cmdlet e le relative descrizioni possono essere ottenute eseguendo `Get-Help command_name`. In alternativa, è possibile vedere anche https://["Guida di riferimento al cmdlet del software SnapCenter"^].
...
. Scaricare il file di metadati della federazione ad FS da "https://[] FQDN>/FederationMetadata/2007-06/FederationMetadata.xml".
